...
namespace Movement.Job {
    public struct RunJob : IEcsUnityJob<VelocityComponent, RunRequestComponent> {
        ...
        public void Execute(int index) {
            ...
            RunRequestComponent runRequestComponent = pool2[pool2Idx];

            if (!runRequestComponent.IsFinish) {
                runRequestComponent.Update(deltaTime);
                velocityComponent.velocity += runRequestComponent.GetVelocity(deltaTime);
            }
